...
- **Auth (JWT bearer)**: stateless. `POST /user/login`, `/user/register`, and `/user/become-instructor` return `{ message, token, data, permissions }`; the SPA stores `token` in `localStorage` and sends it on every request as `Authorization: Bearer <token>`. Token lifetime is `JWT_EXPIRES_IN` (default `7d`). `auth_middleware` verifies the Bearer JWT and checks a Redis-cached **role permission version**; if the role's `lastPermissionUpdate` is newer than the token's, it returns 403 ("Permissions updated. Please login again"). Changing permissions calls `invalidateRolePermissions(roleId)`. Redis is only a cache, so it degrades to Postgres if down. **Logout is client-side** (the SPA drops the stored token); a stateless JWT stays valid until it expires (a documented trade-off, no server-side revocation). There are no cookies and no CSRF (a Bearer header is never auto-sent by the browser).
  - Email-backed flows (`forgot-password`/`reset-password`) are token-in-body flows (unrelated to the auth transport) and live in the user controller + `services/email-service.ts`.
