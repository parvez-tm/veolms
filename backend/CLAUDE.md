...
- **Auth**: `auth_middleware` verifies the JWT and checks a Redis-cached **role permission version**; if the role's `lastPermissionUpdate` is newer than the token's, it returns 403 (re-login). Changing permissions calls `invalidateRolePermissions(roleId)`. Redis is a cache — it degrades to Postgres if down.
...
- **Roles**: `Admin`, `Instructor`, `Student` (seeded). Domain authorization uses
  ...
  **ownership**: instructors may only modify their own courses — enforced by
  `loadOwnedCourse(courseId, req.user)` ([routes/lms/course-access.ts](src/routes/lms/course-access.ts)), used by the course,
...
- **Secure playback**: `GET /lesson/getPlayback/:id` is gated by `assertLessonAccess`
  ...
  presigned MP4 — fallback before transcode finishes / if ffmpeg absent), or
...
- **Encrypted HLS (anti-download) + ABR** — [services/hls-service.ts](src/services/hls-service.ts): after `confirmUpload`, a
  ...
  HLS — variant playlists + a `master.m3u8` (`hls/<assetId>/` in R2) — stores the 16-byte key
  ...
  screen-capture always works — true DRM (Widevine/FairPlay) is the only full stop.
...
## Media lifecycle — avoid orphans (important)

R2 objects and `media_assets` rows must be deleted together. **Always go through
[services/media-service.ts](src/services/media-service.ts)** (`purgeAsset` / `purgeAssetsByIds`) — it deletes the R2
...
- Residual untracked case: a presigned video PUT that lands but is never confirmed leaves
  an R2 object the DB doesn't track — backstop with an **R2 lifecycle rule** on the
  `videos/` prefix.
...
- **Entitlement is perpetual**: `create-order` re-grants enrollment for free if a `paid`
  Payment already exists for `(user, course)` — unenroll→re-enroll never double-charges. It
...
- `MediaAsset.uploadedById` is `SET NULL` on user delete (don't switch to CASCADE — it
...
- **Express 5**: rejected promises auto-forward to the error handler (asyncHandler is redundant but kept); `req.body` is `undefined` (not `{}`) when unparsed — always use `req.body ?? {}`.
