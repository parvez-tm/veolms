  ...
  return (
    <div className="flex min-h-svh bg-background text-foreground">
      ...
      <aside className="hidden w-64 shrink-0 flex-col border-r border-border bg-white md:flex">
        <Link
          to="/"
          className="flex h-16 items-center gap-3 border-b border-border px-5 font-extrabold"
        >
      ...
      <div className="flex flex-1 flex-col">
        ...
        {/* Mobile nav */}
        <div className="flex gap-2 overflow-x-auto border-b border-border bg-white px-4 py-2 md:hidden">
          {nav.map((item) => (
            <NavLink key={item.to} to={item.to} end={item.end} className={itemClass}>
              <item.icon className="h-4 w-4" />
              {item.label}
